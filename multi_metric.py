...
#scoring = {'AUC': 'roc_auc', 'Accuracy': make_scorer(mean_squared_error)}
...
scoring1 = {'EVS': make_scorer(explained_variance_score, greater_is_better=False), 'R2S': make_scorer(r2_score)}
...
gs = GridSearchCV(DecisionTreeRegressor(random_state=42),
                  ...
                  scoring=scoring1, cv=5, refit='R2S')
gs.fit(X, y['Score'].values)
results2 = gs.cv_results_
...
X_axis = np.array(results2['param_min_samples_split'].data, dtype=float)

for scorer, color in zip(sorted(scoring1), ['g', 'k']):
    for sample, style in (('train', '--'), ('test', '-')):
        sample_score_mean = results2['mean_%s_%s' % (sample, scorer)]
        sample_score_std = results2['std_%s_%s' % (sample, scorer)]
    ...
    best_index = np.nonzero(results2['rank_test_%s' % scorer] == 1)[0][0]
    best_score = results2['mean_test_%s' % scorer][best_index]
